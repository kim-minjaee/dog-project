...
public class AdminService {
	...
	public ModelAndView adoptManagement(HttpSession session, ListDto list) {
		...
		List<AdoptionListDto> dogInfo = adDao.getDoginform(list); // get dog info + Dog adoption status

		mv.addObject("adList", dogInfo);

		list.setPageNum(pageNum);

		String pageHtml = getAdminPaging(list);
	...
	private String getAdminPaging(ListDto list) {
		log.info("admin.getPaging()");

		String pageHtml = null;

		int maxNum = adDao.adminCountSelect(list);
		int pageCnt = 5;
		String listName = "adoptManagement?";
	...
	public String adoptionDecline(int dogNum, RedirectAttributes rttr) {
		...
		try {
			...
			view = "redirect:/adoptManagement?pageNum=1";
			msg = "입양 거절";
	...
	public ModelAndView adminApplicationList(AdoptionListDto apDto, ListDto list) {
		mv = new ModelAndView();
		...
		List<AdoptionListDto> appDto = adDao.adminApplicationList(list);
		
		list.setPageNum(pageNum);
		String pageHtml = getApplicationPaging(list);
	...
	private String getApplicationPaging(ListDto list) {
		log.info("serv.getPaging()");

		String pageHtml = null;

		int maxNum = adDao.adminApplicationCountSelect(list);
		int pageCnt = 5;
		String listName = "adminApplicationList?";
		PagingUtil paging = new PagingUtil(maxNum, list.getPageNum(), listCnt, pageCnt, listName);
		pageHtml = paging.makePaging();
