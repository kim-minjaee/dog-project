	...
	<section>
		...
		<h2 class="login-header">${productInfo.pname}</h2>
			...
			<form action="./updateProduct" method="post" enctype="multipart/form-data">
				<div class="contain1">
					<div class="photo_wrap">
						...
						<div class="contain1_photo">
							<img class="preview_big_exist" id="preview_big" src="./resources/upload/${pfList[0].sysname}" alt="">											
						</div>											
						<div  class="contain1_photo_thumbnail">
							<c:forEach var="pf" items="${pfList}" varStatus="st">
							<div><img class="thumbnail_img_exist" id="preview_thumb${st.count}" src="./resources/upload/${pf.sysname}" alt="" ></div>
							</c:forEach>
							<c:if test="${pfList.size() < 4}">
								<c:forEach var="cnt" begin="1" end="${4-pfList.size()}" step="1">
								<div><img class="thumbnail_img_nothing" id="preview_thumb${cnt+pfList.size()+1}" src="./resources/images/no_image.png" alt="" ></div> 
								</c:forEach>
							</c:if>		
						</div>
						<div>
							<div>* 현재 저장된 사진 : </div>
							<c:forEach var="pf" items="${pfList}" varStatus="st">
								<input type="hidden" name="saveThumbnail${st.count}" value="${pf.sysname}">${pf.sysname}, 
							</c:forEach>
						</div>		
					...
					<div class="contain1_product_title" >
						...
						<select name="pcategory" id="pcategory">
							<option value="food" <c:if test="${product.pcategory=='food'}">selected</c:if>> 사료 </option>
							<option value="clothes" <c:if test="${product.pcategory=='clothes'}">selected</c:if>> 옷/목줄 </option>
							<option value="cage" <c:if test="${product.pcategory=='cage'}">selected</c:if> > 케이지</option>
							<option value="toy" <c:if test="${product.pcategory=='toy'}">selected</c:if>> 장난감 </option>
						</select>
				
						</div>
						
						<div ><span class="title" style="letter-spacing: 24px">상품명</span> : &nbsp;<input type="text" name="pname" id="pname" value="${product.pname}"></div>
						<div ><span class="title" style="letter-spacing: 24px">판매가</span> : &nbsp;<input type=text name="pprice" id="pprice" value="${product.pprice}" onblur="ppointSelect()"></div> <!-- onblur : 포커스를 잃었을때 이벤트 발생됨-->
						<div ><span class="title" style="letter-spacing: 24px">적립금</span> : &nbsp;<input type=text name="ppoint" id="ppoint" value="${product.ppoint}" readonly></div>
						<div ><span class="title" style="letter-spacing: 30px">수 량</span> : &nbsp;<input type="text" name="pquantity" id="pquantity" value="${product.pquantity}"></div>						
					
						
						<hr>
						
						<c:forEach var="op" items="${poList}" varStatus="st">
						<input type="hidden" name="optionnum" id="optionnum" value="${op.optionnum}">
						<div class="contain1_product_title_option${st.count}">
							<div ><span class="title" style="letter-spacing: 24px">옵션${st.count}</span> : &nbsp; 
								<input type="text" name="poption" id="poption" value="${op.poption}"></div>
							<div><span class="title, list" style="letter-spacing: 24px">A</span> : &nbsp; 
								<input type=text name="choiceA" id="choiceA" value="${op.choiceA}"></div>
							<div><span class="title, list" style="letter-spacing: 24px">B</span> : &nbsp; 
								<input type=text name="choiceB" id="choiceB" value="${op.choiceB}"></div>
							<div><span class="title, list" style="letter-spacing: 24px">C</span> : &nbsp; 
								<input type=text name="choiceC" id="choiceC" value="${op.choiceC}"></div>
						</div>	
						</c:forEach>
			 ...
			 <%--end :class contain1 --%>
							<input type="hidden" name="pnum" id="pnum" value="${product.pnum}"></div>	
					<hr>
					
					<div class="contain2">
					<div ><textarea rows="15" cols="90" name="pinfo" id="pinfo" placeholder="상품설명 ">${product.pinfo}</textarea></div> 
					...
					<br>
						...
						<input type="submit" value="수정"style="height: 30px"></input>
						</div>
