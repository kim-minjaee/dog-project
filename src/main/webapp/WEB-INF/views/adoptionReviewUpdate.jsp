...
<body>
	...
	<div class="container">
		<div class="be-comment-block">
			...
			<form action="./adoptionReviewSetUpdate" method="post"
				enctype="multipart/form-data">
				<div class="row">
					<div class="col-xs-12 col-sm-6">
						<div class="form-group fl_icon">
							<input type="hidden" name="memberId" value="${rList.memberId}">
							<input type="hidden" name="dogNum" value="${rList.dogNum}">
							<input type="hidden" name="drNum" value="${rList.drNum}">
						</div>
					</div>
					...
					<div class="col-xs-12">
						<div class="form-group">
							<input class="form-input" name="drTitle" type="text"
								placeholder="제목을 입력하세요." value="${rList.drTitle}"  required>
							<textarea class="form-input" name="drContent"
								placeholder="내용을 입력하세요." rows="25" required>${rList.drContent}</textarea>
						</div>
					</div>
					<hr>
					<div class="filebox">
				<div id="bfile" style="margin-bottom: 10px;">
					...
					<c:if test="${!empty pList}">
						<c:forEach var="f" items="${pList}">
							<label style="width: 50%;" onclick="del('${f.sysname}')">
								${f.oriname} </label>
				...
				<label for="file">파일 추가</label> <input type="file" name="files"
					...
					value="파일선택" readonly> <input type="hidden" id="filecheck"
					value="0" name="fileCheck">
			</div>
			<div class="regbutton" style="text-align: center;">
				<input type="submit" name="files" class="button1" value="수정">
				<input type="submit" formaction="./deleteReview" name="files" class="button1" value="삭제">
